...
### 5.4 資料一致性與展示可信度（剩餘項目）

**目標**：移除或標示仍屬示範性質的假資料，令比賽展示更可信。

- [ ] 個人中心的 NFC 感應金鑰由固定 `NFC-SEC-AA8281` 改為實際 `nfcTagId`。
- [ ] 單車詳情「最近定位：沙田公園 (200米內)」為假資料，需移除或標示為示範。
- [ ] ReportTab 的示範照片、GPS 位置及回收站 / 環保夥伴資料標示為「示範資料」或接入真實來源。
- [ ] 減碳距離可能因重複按「導航完成」而過度累加；加入行程紀錄列表或去重 / 上限機制。

**驗收標準**：UI 不把示範資料誤表達為即時真實資料；NFC 編號、車架編號與個人中心顯示一致。

### 5.5 套件清理與效能（語言部分已完成）

**目標**：減少不必要體積、降低打包警告。

- [ ] 移除未使用依賴：`@google/genai`、`@vis.gl/react-google-maps`、`@types/google.maps`、
  `express`、`@types/express`、`dotenv`（移除後需 `npm i` 並重新 build 驗證）。
- [ ] 主 bundle 仍約 600 kB，評估對地圖 / Firebase 進一步 `manualChunks` 或路由層級 lazy load。
- [ ] 補充最少量 smoke test 或「比賽前檢查表」，列出開 App → 地圖載入 → 登記 → 舉報 → 個人中心的手動驗證步驟。

**驗收標準**：`package.json` 只保留實際使用依賴；build 大型 chunk 警告減少；主要頁面仍正常載入。
