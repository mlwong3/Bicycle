...
> 專案：mlwong3/Bicycle ・ Firebase：bicycle-ee76c ・ 最後更新：2026-07-13
...
| # | 項目 | 內容 | 狀態 |
|---|------|------|------|
| 30 | 管理員系統 Phase 1 | Report 加入座標、處理狀態、時間線與處理人；市民舉報保存 GPS；相片優先嘗試 Firebase Storage，失敗保留原圖；新增示範管理員工作台、登入 session、案件狀態流轉及市民端「待審核／處理中／已清理／不成立」同步顯示。AI 僅保留獨立試運行，巡邏路線尚未納入實際方案。 | ✅ 完成（AI／巡邏待評估） |
| 31 | 跨部門個案管理 MVP | 依新方案加入 13 狀態案件模型、公開／管理員 adapter、六項人工 rubric、可配置程序快照及期限、AI 文字／欄位規則式分類 fallback、5–8 宗巡查次序（優先度 + 最近鄰居 + 2-opt）、管理員確認後才排程，以及市民端案件類別及位置來源。AI 不讀取相片；路線只作示範性建議。 | ✅ 完成（正式 Auth／Rules／後端 AI 待後續） |
...
| 13 | 收藏與紀錄可點擊詳細頁 | 個人中心「示範收藏路線 / 收藏泊位 / 舉報紀錄」三張卡片改為可點擊，各自開詳細彈窗（舉報列出真實紀錄、泊位列出收藏清單並可跳地圖、路線列示範資料） | ✅ 完成 |
| 14 | 真正連上 Firebase | 將 `bicycle-ee76c` 前端 config 內嵌至 `src/firebase.ts` fallback，部署版實際連 Firestore + 匿名登入；推上 GitHub main 觸發自動部署 | ✅ 完成 |
...
| 21 | 地圖真實 GPS + 10km/h 時間 | MapTab 進場自動取得真實 GPS 位置（藍點、泊位距離、導航起點、「我的位置」皆改用真實座標，失敗退回沙田）；導航面板新增「全程距離」與「預計時間（以 10 km/h 估算）」兩格 | ✅ 完成 |
...
| 28 | ESP32 函式庫改版：遷移到 FirebaseClient | 原本使用的 Firebase-ESP-Client（及更舊的 Firebase-ESP32）已被作者標示 `[DEPRECATED]`，統一導向新版非同步函式庫 `FirebaseClient`。查閱官方 GitHub 的 Anonymous/Set/Push 範例後，把 `esp32/parking_sensor.ino` 全面改寫成新版 API（`UserAccount`+`signup()` 匿名登入、`JsonWriter`/`object_t` 組 JSON、`Database.set<object_t>()`/`Database.push<object_t>()` await 模式讀寫），`esp32/README.md` 同步更新函式庫安裝步驟與常見錯誤對照表 | ✅ 完成 |
| 29 | NFC 標籤私隱優先格式 + 防盜驗證讀取 | 依 4.3 節私隱優先設計，`src/nfc.ts` 的 `BikeTagData` 改為 `{ tagId, bikeId, frameNo, appUrl }`——標籤只寫入編號與 App 網址、**不含姓名/型號**（車主資料只留在帳戶）；`writeBikeTag` 改寫 url + json 兩筆記錄，感應未安裝 App 的手機也能開啟網址。`NfcTab` 登記時寫入此格式，並新增「防盜驗證・感應標籤讀取」功能（真 NFC `readBikeTag` + 結果彈窗），使外部工具寫好的實體卡可被 App 掃描驗證。新增 `nfc_cards/` 內 10 份示範卡 JSON（QJ-NFC-001~010）與登記表 README | ✅ 完成 |
...
> 本章只保留尚未完成或需要下一輪實作的工作。已完成項目（開放數據快取 / 節流、localStorage
> 版本化與容錯、移除 `alert()`、README / metadata / title 更新、Firebase 輕量後端骨架、
> MapTab 抽出 ParkingInfoCard / NavigationPanel、收藏路線改名示範並加入可點擊詳細頁、
> 只保留繁體中文、`firebase-deploy.yml` 注入 `VITE_FIREBASE_*`）已移至 Changelog，不再列入本章。
>
> 最後整理：2026-06-27。
...
### 5.4 資料一致性與展示可信度（已完成，剩餘觀察項）

> 主要項目已完成（見 Changelog 16）：NFC 金鑰改真實 `nfcTagId`、最近定位標示範、
> ReportTab 回收站 / 環保夥伴標「示範資料」、減碳單次上限。以下為可選的進一步強化：

- [ ] ReportTab 的示範照片、GPS 位置若有條件可接入真實來源（目前已標示為示範，屬可接受）。
- [ ] 減碳改為完整「行程紀錄列表」，可逐筆檢視與刪除（目前為上限防灌水）。

### 5.5 效能（套件清理已完成）

> 未使用依賴已移除（見 Changelog 17）。以下為剩餘效能項：

- [ ] 主 bundle 仍約 600 kB；評估對地圖 / Firebase 進一步 `manualChunks` 或頁面層級 lazy load。
- [ ] 補充最少量 smoke test 或「比賽前檢查表」，列出開 App → 地圖載入 → 登記 → 舉報 → 個人中心的手動驗證步驟。

**驗收標準**：build 大型 chunk 警告減少；主要頁面仍正常載入。

### 5.6 真正連上 Firebase（部署實測）

**目標**：把「程式已就緒但未連線」變成「部署版實際讀寫 Firestore」。

- [ ] 在 Firebase Console 註冊 Web App，取得 `firebaseConfig`（apiKey 等，屬可公開的前端設定）。
- [ ] 啟用 Anonymous Authentication 與 Cloud Firestore，部署 `firestore.rules`。
- [ ] 將設定接上：以 GitHub Secrets 注入 `VITE_FIREBASE_*`（workflow 已支援），或直接寫入
  `src/firebase.ts` 的 fallback（前端 config 可公開，安全性由 Firestore 規則把關）。
- [ ] 實測登記單車 / 舉報 / 導航後，Firestore 是否出現對應 `bikes` / `nfcTags` / `reports` / `trips`。

**驗收標準**：部署網址操作後，Firestore 主控台可見對應文件；未設定時仍能以 localStorage 優雅降級。
...
- [ ] `useLeafletMap`：地圖初始化、底圖切換、縮放控制。
- [ ] `useCyclingData`：運輸署泊位 / 單車徑載入與快取。
- [ ] `MapControls`：搜尋列、CSDI key 面板、圖層選單。
- [ ] 評估加入 CYCRAMP 單車斜道圖層、低縮放「載入此區域」按鈕。

**驗收標準**：`MapTab.tsx` 進一步縮短、行為不變、`npm run lint` 與 build 通過。
